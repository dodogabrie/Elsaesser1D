...
subroutine RHS()
  ...
  call DFS1(aux_f, der1_f)
  call DFS1(aux_g, der1_g)
  do i = 1, n
    aux1_f(i) = aux_f(i) - aux_g(i)
  end do
  call DFS2(aux1_f, der2_f)
  do i = 2, n-1
    tmp_f = aux_f(i)
  ...
  tmp_f = aux_f(1)
  lambda_f = c + aux_g(1)
  aux_f(1) = ( 1 + SIGN(1.d0, lambda_f) ) * lambda_f/2 * der1_f(1)
  lambda_g = - c + tmp_f
  aux_g(1) = ( 1 + SIGN(1.d0, lambda_g) ) * lambda_g/2 * der1_g(1)

  tmp_f = aux_f(n)
  lambda_f = c + aux_g(n)
  aux_f(n) = ( 1 - SIGN(1.d0, lambda_f) ) * lambda_f/2 * der1_f(n)
  lambda_g = - c + tmp_f
  aux_g(n) = ( 1 - SIGN(1.d0, lambda_g) ) * lambda_g/2 * der1_g(n)
end subroutine RHS
...
program main
  ...
  state = 0
  do i = 1, n
    f(i) = EXP( -( xx(i) - mu_f)*( xx(i) - mu_f)/(sigma * sigma) )! * SIN( kk *  2 * PI / L * xx(i))
    g(i) = 0!EXP( -( xx(i) - mu_g)*( xx(i) - mu_g)/(sigma * sigma) )! * SIN( kk * 2 * PI / L * xx(i))
  end do
