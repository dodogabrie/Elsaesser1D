...
program main
   use lele_filter, only: pentadiag_simple
   ...
   real*8, dimension(:), allocatable :: a_lolo, a_upup
   ...
   allocate(a_lolo(n-2), a_upup(n-2))
   do i = 1, n
      a_diag(i) = d
      if (i < n) then
         a_lo(i) = c
   ...
   call cpu_time(start)
   call pentadiag_simple(e, c, d, aa, b, y, sol_p, n )
   call cpu_time(finish)
   print '("Linear pentadiag Time = ",f6.3," seconds.")',finish-start

   call cpu_time(start)
   call inv(A, Ainv, n)
   sol_i = matmul(Ainv, y)
   call cpu_time(finish)
   print '("Lapack inverse pentadiag Time = ",f6.3," seconds.")',finish-start
